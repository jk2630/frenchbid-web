...
const FBHeader = (props) => {
    ...
    const {display_stats, display_menu} = props;
    ...
    >
        <div className="flex items-center gap-3 shrink-0">
            <img src={fbLogo} alt="French Bid Logo" className="h-10 w-15 rounded" />
             <div className="hidden md:block">
                <p className="text-sm text-gray-300 italic">"Bid with wit, play with heart — every card tells a story."</p>
            </div>
        </div>
        <div className="flex items-center flex-wrap justify-center gap-x-4 gap-y-2 text-md sm:text-lg">
            ...
            {display_menu && <span className="flex items-center gap-2 mx-4 md:mx-12"><FBMenuDropDown /></span>}
        </div>
    </motion.header>
)};
